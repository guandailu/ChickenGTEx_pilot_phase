library(data.table)
library(tidyverse)
df = fread("BRICD5 - chicken FI - human weight - phewas.csv")
...
df$Domain = ifelse(df$Domain != "Environmental", df$Domain, "Environment")
library(ggrepel)
p=ggplot(df)+
...
ggsave("BRICD5 - chicken FI - human weight - phewas.csv.pdf", p, width=5, height=8)