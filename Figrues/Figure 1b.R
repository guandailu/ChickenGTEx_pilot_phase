...
df=fread(paste0("ChickenGTEx_leafcutter_perind.counts.gz.qqnorm_chr",i))
df=as.data.frame(df)
DF=rbind(DF,df)
}
rownames(DF) = DF$ID
DF[,1:4]=NULL
names(DF)=str_replace(colnames(DF),"_rg.bam.chr","")
...
#### Tree clustering using pearson correlation of samples
res <- cor(DF[1:1000,],method="pearson")
dist <- as.dist(1 - res)
tree=hclust(dist, method="complete")
save(tree, file = "Chicken_GTEx_leafcutter_splicing_tree.Rdata")

#### Color settings
tissue_colors <- read.table("/group/zhougrp2/dguan/07_stringtie_quant/gene_atlas//Chicken_GTEx_tissue_color_setting.txt", header = T, sep = "\t", comment.char = "@")
colors <- as.vector(tissue_colors$color)
names(colors) <- as.vector(tissue_colors[match(colors, tissue_colors$color),1])
samples$colors <- colors[match(samples$Tissue_categories,names(colors))]
ann_colors=samples[match(tree$labels, samples$BioSample),"colors"]
...
#load("Chicken_GTEx_leafcutter_splicing_tree.Rdata")
#### Tree viz
p=ggtree(tree, layout="fan", open.angle=90)+
    theme(legend.position='none') +
        geom_tippoint(color=ann_colors, shape=124, size=4)

pdf("Chicken_GTEx_leafcutter_splicing_tree.pdf", width=8, height=8)
print(p)
dev.off()
