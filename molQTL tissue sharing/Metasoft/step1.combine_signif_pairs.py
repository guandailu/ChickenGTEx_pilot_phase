...
parser.add_argument('snp_map', help="File listing snp coordinates")
...
dfs = []
for f in file_paths:
    dfs.append(pd.read_csv(f, sep='\t', usecols=['variant_id', 'phenotype_id']))
dfs = pd.concat(dfs, axis=0)
dfs = dfs.drop_duplicates()

snps = pd.read_csv(args.snp_map, sep='\t', usecols=['variant_id', 'chr', 'pos'])
dfs = dfs.merge(snps, left_on='variant_id', right_on='variant_id')
